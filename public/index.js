...
let checkVar = 0;
let isFota = false;
const fotaBtn = document.querySelector("#fota-btn");
const fotaDiv = document.querySelector("#fota");
const processDiv = document.querySelector("#process-div");

fotaBtn.addEventListener("click", () => {
  isFota = !isFota;
  console.log("FOTA mode: ", isFota);
  if (isFota) {
    requestFota();
  }
});

function requestFota() {
  if (isFota) {
    processDiv.innerHTML = "Waiting OTA...";
    processDiv.style.display = "block";
    fotaDiv.style.marginRight = "47.8px";
    checkVar++;
    setTimeout(() => requestFota());
    if(checkVar > 600) {
      processDiv.innerHTML = "OTA Success!!!";
      processDiv.style.display = "block";
      fotaDiv.style.marginRight = "41px";
    }
  }
  if (!isFota) {
    checkVar = 0;
    processDiv.style.display = "none";
    fotaDiv.style.marginRight = "150px";
    setTimeout(() => requestFota());
  }
}
